...
set "OUT=%ROOT%\dump\critical"
set "PL=%ROOT%\preloader\g88_preloader_exploited_from_brom.bin"
set "SKIP=super,userdata,otp,flashinfo"
...
echo Oilsky G88 critical readback.
echo Root:      %ROOT%
echo mtkclient: %MTK%
echo Output:    %OUT%
echo Preloader: %PL%
...
echo This script only reads partitions. It does not flash, erase, format or unlock anything.
echo.
...
if not "%RC%"=="0" (
  echo.
  echo ERROR: mtkclient readback failed with exit code %RC%.
...
:verify
echo.
echo Verifying critical dump sizes, boot_a header and hashes...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "  'vendor_boot_b.bin'=67108864; 'dtbo_b.bin'=8388608; 'tee_b.bin'=8388608; 'vbmeta_b.bin'=8388608;" ^
  "  'vbmeta_system_b.bin'=8388608; 'vbmeta_vendor_b.bin'=14680064" ^
  "};" ^
  ...
  "if($bad.Count){Write-Host 'ERROR: missing files or size mismatch in critical dump'; exit 2};" ^
...
echo.
echo Done. Critical backup command and verification completed successfully.
pause
exit /b 0
